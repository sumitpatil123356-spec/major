  ...
  return (
    <AppShell title="Smart alerts" subtitle="Stay ahead of expiry — we'll keep watch.">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          { t: "Critical", c: "critical", n: items.filter(i => i.s === "critical").length, d: "≤ 7 days" },
          { t: "Warning", c: "warning", n: items.filter(i => i.s === "warning").length, d: "≤ 30 days" },
          { t: "Suggestions", c: "primary", n: items.filter(i => i.donatable).length, d: "Ready to donate" },
        ].map((s) => (
          <Card key={s.t} className="border-border/60">
            <CardContent className="p-5 flex items-center gap-4">
              <div className={cn(
                ...
                {s.c === "primary" ? <HandHeart className="h-5 w-5" /> : <AlertTriangle className="h-5 w-5" />}
              </div>
              <div>
      ...
      <Card className="mt-6 border-border/60">
        <CardContent className="p-0">
          <div className="divide-y divide-border">
            {items.map((i) => (
              <div key={i.id} className="flex items-center gap-4 px-6 py-4 hover:bg-muted/30 transition-colors">
                <div className={cn(
                  "grid h-10 w-10 place-items-center rounded-xl",
                  i.s === "critical" ? "bg-critical/15 text-critical" : "bg-warning/20 text-warning-foreground"
                )}>
                ...
                <Button size="sm" variant={i.s === "critical" ? "default" : "outline"} className={i.s === "critical" ? "brand-gradient text-white hover:opacity-90" : ""}>
