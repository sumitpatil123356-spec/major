...
import { HandHeart, MapPin, MessageSquare, Loader2 } from "lucide-react";
  ...
  const handleRequest = async (product) => {
    ...
    try {
      ...
      await updateProduct(product.id, {
        donatable: false,
        notes: `${product.notes || ""} (Requested by ${user.full_name})`,
      });
              ...
              <CardContent className="p-5 space-y-3">
                ...
                <div className="flex gap-2 pt-1">
                  ...
                  <Button
                    size="icon"
                    variant="outline"
                    onClick={() => toast(`Messaging option with ${p.owner} coming soon`)}
                  >
          ...
          {donatableItems.length === 0 && (
            <div className="col-span-full py-16 text-center rounded-2xl border border-dashed border-border bg-card/50">
              ...
              <p className="text-sm text-muted-foreground max-w-sm mx-auto mt-1">
                There are currently no items listed for donation. Try adding a product and marking
                it as donatable!
              </p>
            </div>
          )}
