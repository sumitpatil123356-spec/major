    ...
    <AppShell title="Community impact" subtitle="The collective force of every ReShelf action.">
        ...
        {[
          ...
          <Card key={s.l} className="border-border/60">
            <CardContent className="p-7 text-center">
              <div className="font-display text-5xl font-semibold text-brand-gradient tabular-nums">{s.v}{s.suffix ?? ""}</div>
      ...
      <Card className="mt-8 border-0 brand-gradient text-white overflow-hidden shadow-[var(--shadow-glow)]">
        <CardContent className="p-10 md:p-14">
          <Badge className="bg-white/20 text-white hover:bg-white/20"><Sparkles className="mr-1 h-3 w-3" /> Coming soon</Badge>
          <h2 className="mt-4 font-display text-3xl md:text-4xl font-semibold tracking-tight">AI features on our roadmap</h2>
          <p className="mt-2 text-white/85 max-w-xl">Smarter shelves, less waste. Here's what's next.</p>
          <div className="mt-8 grid md:grid-cols-2 gap-4">
            {[
              { i: ScanLine, t: "AI receipt scanner", d: "Auto-add products from a photo of your receipt." },
              { i: Brain, t: "Expiry prediction", d: "Predict shelf life based on storage and brand history." },
              { i: Network, t: "Smart donation matching", d: "Match surplus to NGOs by category, distance and urgency." },
              { i: LineChart, t: "Predictive waste analytics", d: "Forecast next month's waste and prevent it early." },
            ].map((f) => (
              <div key={f.t} className="rounded-2xl bg-white/10 backdrop-blur p-5 border border-white/15">
