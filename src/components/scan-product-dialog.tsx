...
import {
  barcodeDatabase, type BarcodeProduct, type Product, type Category,
} from "@/lib/mock-data";
import { toast } from "sonner";
import { cn } from "@/lib/utils";
import { GroqScanner } from "./GroqScanner";

type Stage = "idle" | "scanning" | "detected" | "confirm";
...
export function ScanProductDialog({
  ...
  const [scanType, setScanType] = useState<"barcode" | "vision">("barcode");
  const [stage, setStage] = useState<Stage>("idle");
  const [match, setMatch] = useState<BarcodeProduct | null>(null);
            ...
            {stage !== "detected" && (
              <div className="relative aspect-[4/3] w-full overflow-hidden rounded-2xl border border-border bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
                ...
                <div className="absolute inset-x-0 bottom-0 p-4 text-center">
                  {stage === "idle" && (
                    <Button onClick={startScan} className="brand-gradient text-white hover:opacity-90">
            ...
            {stage === "detected" && match && (
              <div className="space-y-4">
                <div className="flex items-center gap-2 text-sm text-primary">
                  ...
                  <Badge variant="secondary" className="ml-auto font-mono text-[10px]">{match.barcode}</Badge>
                </div>
                ...
                <div className="flex gap-4 rounded-2xl border border-border bg-card p-4">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="font-display text-lg font-semibold leading-tight">{match.name}</div>
            ...
            <DialogFooter className="gap-2 sm:gap-2 mt-4">
              {stage === "detected" ? (
                <>
                  ...
                  <Button onClick={confirmAdd} className="brand-gradient text-white hover:opacity-90">
